...
export class ContractLoader {
  ...
  async load(address: string): Promise<CollaborativeTrainer> {
    ...
    return Promise.all([
      mainEntryPoint.methods
        .classifier()
        .call()
        .then((classifierAddress: string) => {
      ...
      mainEntryPoint.methods
        .dataHandler()
        .call()
        .then((dataHandlerAddress: string) => {
          return this.getContractInstance({
            abi: DataHandler.abi,
      ...
      mainEntryPoint.methods
        .incentiveMechanism()
        .call()
        .then((incentiveMechanismAddress: string) => {
          return this.getContractInstance({
            abi: IncentiveMechanism.abi,
            address: incentiveMechanismAddress,
          });
        }),
    ]).then(async ([classifier, dataHandler, incentiveMechanism]) => {
      const [classifierOwner, dataHandlerOwner, incentiveMechanismOwner] =
        await Promise.all([
      ...
      if (
        classifierOwner !== address ||
        dataHandlerOwner !== address ||
        incentiveMechanismOwner !== address
      ) {
        throw new Error(
          `The classifer, data handler, and incentive mechanism must be owned by the main interface.\n  Main interface address: ${address}\n  Classifier owner: ${classifierOwner}\n  Data handler owner: ${dataHandlerOwner}\n  IM owner: ${incentiveMechanismOwner}.`
        );
      }

      return new CollaborativeTrainer(
        mainEntryPoint,
        classifier,
        dataHandler,
        incentiveMechanism
