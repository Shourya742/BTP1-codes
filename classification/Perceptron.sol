...
contract DensePerceptron is Classifier64 {
    ...
    function norm(
        int64[] memory data
    ) public pure override returns (uint result) {
        result = 0;
    ...
    function predict(
        ...
        require(
            ...
            "The data must have the same dimension as the weights."
        );
        for (uint i = 0; i < data.length; ++i) {
    ...
    function update(
        ...
        require(
            oneSquared - offset < _norm && _norm < oneSquared + offset,
            "The provided data does not have a norm of 1."
        );
